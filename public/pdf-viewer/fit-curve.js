...
function fitCubic(points, leftTangent, rightTangent, error, progressCallback) {
    ...
    if (maxError < error * error) {
        ...
        for (i = 0; i < MaxIterations; i++) {
            ...
            var _generateAndReport2 = generateAndReport(
                points,
                u,
                uPrime,
                leftTangent,
    ...
    if (
        centerVector.every(function (val) {
            return val === 0
        })
    ) {
...
fitCurve.fitCubic = fitCubic
fitCurve.createTangent = createTangent
